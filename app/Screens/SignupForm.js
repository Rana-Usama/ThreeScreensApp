...
function SignupForm(props) {
  ...
  const handleSigUp = () => {
    ...
    if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(tempfeilds[3].value))) {
      ...
      <ScrollView style={{ backgroundColor: Colors.white, flex: 1 }} >
        <View style={styles.mainContainer}>
          ...
          <View style={styles.navigationContainer} >
            <Ionicons name="arrow-back" size={40} color={Colors.backIcon} style={{ fontSize: RFPercentage(3), fontWeight: "bold" }} onPress={() => props.navigation.navigate("SignupScreen")} />
            <Text style={{ fontSize: RFPercentage(3), left: RFPercentage(3), fontWeight: "bold" }}>
              Sign up
            </Text>
          </View>
